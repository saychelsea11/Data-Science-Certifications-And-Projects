...
if num > 50:
	...
	for i in range(50,loop_end+50,50):
		if i == 50:
			url = API_HOST + SEARCH_PATH + '?term=Chinese&location=' + city + '&limit=' + str(i)
			...
			#print (i)
			url = API_HOST + SEARCH_PATH + '?term=Chinese&location=' + city + '&limit=50&offset=' + str(i-50)
			response = requests.get(url,headers=headers_bearer)
			json_res = response.json()
			bus.extend(json_res['businesses'])
			#print (i)
	
	url = API_HOST + SEARCH_PATH + '?term=Chinese&location=' + city + '&limit=' + str(rem) + '&offset=' + str(i)
	response = requests.get(url,headers=headers_bearer)
	json_res = response.json()
	bus.extend(json_res['businesses'])
else:
	url = API_HOST + SEARCH_PATH + '?term=Chinese&location=' + city + '&limit=' + str(num)
	response = requests.get(url,headers=headers_bearer)
	json_res = response.json()
	bus = [x for x in json_res['businesses']]
...
cur.execute('''DROP TABLE IF EXISTS Chinese ''')

cur.execute('''CREATE TABLE IF NOT EXISTS Chinese 
    (id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, Name TEXT, Rating FLOAT, 
     Price TEXT, Category TEXT, Reviews INTEGER)''')
...
for i in range(len(bus)):
	...
	try:
		cur.execute('''INSERT INTO Chinese (Name, Rating, Price, Category, Reviews) 
