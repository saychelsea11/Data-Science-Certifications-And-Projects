...
ggplot(data=evals,aes(x=language,fill=ethnicity)) +geom_bar(position="fill")

ggplot(data=evals,aes(x=language,y=ethnicity))+geom_point()
```
...
ggplot(data=model,aes(x=.fitted,y=.resid))+geom_jitter()+geom_hline(yintercept=0,linetype="dashed")
ggplot(data=model,aes(x=.resid))+geom_histogram()
```
...
```{r diag-mlr-model}
# type your code for the Exercise here, and Knit
ggplot(data=m_bty_gen,aes(x=.fitted,y=.resid))+geom_jitter()+geom_hline(yintercept=0,linetype="dashed")
ggplot(data=m_bty_gen,aes(x=.resid))+geom_histogram()
```
...
m3 = lm(score ~ rank + gender + language + age + cls_perc_eval + 
    cls_students + cls_level + cls_profs + cls_credits + ethnicity, data = evals)
...
m5 = lm(score ~ rank + gender + language + age + cls_perc_eval + cls_level + cls_profs + cls_credits + bty_avg + ethnicity, data = evals)
